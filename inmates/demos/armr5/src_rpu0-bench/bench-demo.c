...
/* test mapping */
static char *map_addr = (char *)0x3ed00000;
static char *map_addr_end = (char *)(0x3ed00000 + (DEFAULT_MB * 1024 * 1024));
...
static void bench_linear(const char *name, void (*bench)(void))
{
    ...
    printf("[RPU-0] linear %s bandwidth over %u KiB (%u MiB) block\n",
    ...
    while (1) {
		...
		if((u32)(ts_now - ts_end) < HALF_TIMER_RANGE) {
            ...
            printf("[RPU-0] %9.1f MiB/s, %9.1f MB/s\n", to_mib(bw), bw);
...
static void bench_step(const char *name, void (*bench)(size_t, size_t), size_t step, size_t step_offset, int step_all)
{
    ...
    printf("[RPU-0] step1 %s bandwidth over %u KiB (%u MiB) block, step %u, offset %u\n",
...
static void bench_auto(const char *name, void (*bench)(size_t, size_t), int step_all)
{
    ...
    printf("[RPU-0] worst-case %s bandwidth over %u KiB (%u MiB) block\n",
           name, map_size / 1024, (map_size + 1024*1024-1) / 1024 / 1024);
    ...
    while (1) {
        ...
        printf("[RPU-0] step %9u: %9.1f MiB/s, %9.1f MB/s\n", step, to_mib(bw), bw);
    ...
    printf("[RPU-0] slowest step size: %u\n", min_step);
}
