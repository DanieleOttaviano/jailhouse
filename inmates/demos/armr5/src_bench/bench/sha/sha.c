...
void sha_byte_reverse( LONG_SHA *buffer, int count )
{
  int i;
  BYTE ct[ 4 ], *cp;

  count /= sizeof( LONG_SHA );
  cp = ( BYTE * ) buffer;
  _Pragma( "loopbound min 16 max 16" )
  for ( i = 0; i < count; ++i ) {
    ct[ 0 ] = cp[ 0 ];
...
{
  if ( ( sha_info->count_lo + ( ( LONG_SHA ) count << 3 ) ) < sha_info->count_lo )
    ++sha_info->count_hi;
  sha_info->count_lo += ( LONG_SHA ) count << 3;
  sha_info->count_hi += ( LONG_SHA ) count >> 29;
...
{
  int count;
  LONG_SHA lo_bit_count, hi_bit_count;
